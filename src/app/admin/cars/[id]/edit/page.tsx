...
import { XIcon, UploadIcon, ImageIcon, Trash2Icon } from "lucide-react";
...
export default function AdminCarEditPage() {
  ...
  useEffect(() => {
    const fetchCar = async () => {
      ...
      if (data) {
        ...
        setMonthlyPrice(car.monthly_price);
        setMinMonths(car.min_months);
        setFuel(car.fuel);
        setTransmission(car.transmission);
        setColor(car.color);
        setServiceType(car.service_type);
        setIncludes(car.includes ?? []);
        setFeatures(car.features ?? []);
        setImages(car.images ?? []);
        setIsVisible(car.is_visible);
        setIsFeatured(car.is_featured);
        setIsSale(car.is_sale);
      }
      setFetching(false);
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!error) {
      ...
      alert("차량 수정에 실패했습니다: " + error.message);
    }
  ...
  const handleDelete = async () => {
    if (!confirm("이 차량을 삭제하시겠습니까? 이 작업은 되돌릴 수 없습니다.")) return;
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <div className="mb-4 flex items-center justify-between">
        <h2 className="text-lg font-semibold">차량 수정</h2>
